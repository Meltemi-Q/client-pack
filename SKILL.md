...
description: |
  ...
  Empty PC: run scripts/bootstrap_pack_tools.ps1 or 一键安装打包工具.cmd (Miniconda + Inno Setup 6.7.3). PyInstaller is pip-installed by setup_build_env.bat as pyinstaller==6.20.0, never as a standalone exe.
...
`conda.bat` and `ISCC` must be PASS. Read `Found spec:` / `Found exe name in spec:` from the check output.

### B) Pack

8. `NOW`: check_pack_env.ps1. Exit `0` READY, `2` NEEDS_SETUP, `1` FAIL.
9. `ACT`: FAIL on conda/ISCC → A. Other FAIL → `references/pitfalls.md`.
10. `ACT`:
...
4. Conda env `golgi-build`, Python **3.8.20 64-bit** (pack script asserts Python **3.8**).
5. `setup_build_env.bat` pip-installs `pyinstaller==6.20.0`, PySide6 6.6.2, shiboken6 6.6.2.
6. `build_and_pack.bat` using **this checkout's** onedir spec and `installer.iss`.

Optional env: `GOLGI_NOPAUSE=1`, `GOLGI_FAST_BUILD=1`, `GOLGI_BUILD_ENV`, `GOLGI_ISCC`.
